...
var _nodeValue = func(parent, path, default = nil) {
  if ((parent == nil)) {
    return default;
  }
  var n = parent.getNode(path);
  if ((n == nil)) {
    return default;
  }
  return (n.getValue() or default);
}
var _nodeText = func(parent, path, default = "") {
  var v = _nodeValue(parent, path, default);
  return ((v == nil) ? default : v);
}
var _try = func(f, onErr = nil) {
  var errs = [];
  call(f, [], nil, {}, errs);
  if ((size(errs) > 0)) {
    if (((onErr != nil) and (typeof(onErr) == "func"))) {
      call(onErr, [errs], nil, {}, []);
    }
    return 0;
  }
...
var download = func(username, onSuccess, onFailure = nil) {
  ...
  if ((onFailure == nil)) {
    onFailure = func(r) {
      ...
      logprint(4, sprintf("SimBrief download from %s failed with HTTP status %s", url, r.status));
    }
  }
  http.save(url, filename).done(func(r) {
...
var toFlightplan = func(ofp, fp = nil) {
  ...
  var departureID = _nodeText(ofp, "origin/icao_code", "");
  if ((departureID == "")) {
    logprint(5, "Missing origin/icao_code in OFP");
    return nil;
  }
  ...
  var destinationID = _nodeText(ofp, "destination/icao_code", "");
  if ((destinationID == "")) {
    logprint(5, "Missing destination/icao_code in OFP");
    return nil;
  }
  ...
  var initialAltitude = _nodeValue(ofp, "general/initial_altitude", nil);
  var cruiseAltitude = initialAltitude;
  var seenTOC = 0;
  var wps = [];
  var ofpNavlog = ofp.getNode("navlog");
  if ((ofpNavlog == nil)) {
    logprint(5, "Missing navlog node in OFP");
    return nil;
  }
  var ofpFixes = ofpNavlog.getChildren("fix");
  if (((ofpFixes == nil) or (size(ofpFixes) == 0))) {
    logprint(4, "No fixes in OFP navlog");
  }
  var sidID = nil;
  var starID = nil;
  ...
  forindex (var fixIndex; ofpFixes) {
    ...
    _try(func() {
      var isSidStar = (_nodeValue(ofpFix, "is_sid_star", 0) or 0);
      if ((isSidStar == 1)) {
        if ((((_nodeText(ofpFix, "stage", "") == "CLB") and (getprop("/sim/simbrief/options/import-departure") or 0)) and (sidID == nil))) {
          sidID = ofpFix.getValue("via_airway");
        } elsif ((((_nodeText(ofpFix, "stage", "") == "DSC") and (getprop("/sim/simbrief/options/import-arrival") or 0)) and (starID == nil))) {
          starID = ofpFix.getValue("via_airway");
        }
        return nil;
      }
      var ident = _nodeText(ofpFix, "ident", "");
      if ((ident == "")) {
        logprint(4, sprintf("Skipping fix #%d: missing ident", fixIndex));
        return nil;
      }
      ...
      var lat = _nodeValue(ofpFix, "pos_lat", nil);
      var lon = _nodeValue(ofpFix, "pos_long", nil);
      if (((lat == nil) or (lon == nil))) {
        logprint(4, sprintf("Skipping fix #%d (%s): missing lat/lon", fixIndex, ident));
        return nil;
      }
      ...
      if ((wp == nil)) {
        logprint(4, sprintf("Skipping invalid waypoint object for fix #%d (%s)", fixIndex, ident));
        return nil;
      }
      if (importAirways) {
        var airway = ofpFix.getValue("via_airway");
        if ((((airway != nil) and (airway != "")) and (airway != "DCT"))) {
          wp.simbrief_airway = airway;
        }
      }
      append(wps, wp);
      if ((((seenTOC and (initialAltitude != nil)) and (alt != nil)) and (alt == initialAltitude))) {
        seenTOC = 0;
      } elsif ((((alt != nil) and (cruiseAltitude != nil)) and (alt > cruiseAltitude))) {
        cruiseAltitude = alt;
      }
    }, func(errs) {
      var ident2 = (((ofpFix != nil) and (ofpFix.getValue("ident") != nil)) ? ofpFix.getValue("ident") : "unknown");
      logprint(4, sprintf("Error parsing fix #%d (%s)", fixIndex, ident2));
      debug.printerror(errs);
    });
  }
  if ((fp == nil)) {
  ...
  if ((getprop("/sim/simbrief/options/import-departure") or 0)) {
    var departureRunwayID = ofp.getNode("origin").getValue("plan_rwy");
  ...
  if ((getprop("/sim/simbrief/options/import-arrival") or 0)) {
    var destinationRunwayID = ofp.getNode("destination").getValue("plan_rwy");
    logprint(2, sprintf("Trying to select arrival: %s / %s", (starID or "NONE"), destinationRunwayID));
...
var importFOB = func(ofp) {
  ...
  var allocate = func(tankNumber, maxAmount = nil) {
    ...
    var tankCapacity = ((tankNode.getNode("capacity-m3").getValue() or 0) * (tankNode.getNode("density-kgpm3").getValue() or 0));
  ...
  var allocatePair = func(tank1, tank2) {
    ...
    var cap = (cap1 + cap2);
    allocate(tank1, ((unallocated * cap1) / cap));
    allocate(tank2, ((unallocated * cap2) / cap));
  }
  if ((strategy == "first-come-first-serve")) {
    ...
    while ((i < numTanks)) {
      ...
      if (((((i >= (numTanks - 1)) or string.imatch(tankName, "*center*")) or string.imatch(tankName, "*front*")) or string.imatch(tankName, "*rear*"))) {
        allocate(i);
        i = (i + 1);
      } else {
        allocatePair(i, (i + 1));
    ...
    foreach (var tankNode; tankNodes) {
      totalCapacity = (totalCapacity + (tankNode.getValue("capacity-m3") or 0));
    }
    for (var i = 0; (i < numTanks); i += 1) {
      var capacity = (tankNodes[i].getValue("capacity-m3") or 0);
...
var importPayload = func(ofp) {
  ...
  foreach (var node; weightNodes) {
    ...
    if ((((((string.imatch(nodeName, "*passenger*") or string.imatch(nodeName, "*pax*")) or string.imatch(nodeName, "*cabin*")) or string.imatch(nodeName, "*class*")) or string.imatch(nodeName, "*baggage*")) or string.imatch(nodeName, "*seat*"))) {
      append(paxWeightNodes, node);
    } elsif ((string.imatch(nodeName, "*cargo*") or string.imatch(nodeName, "*payload*"))) {
      append(cargoWeightNodes, node);
    }
  }
  if (((size(paxWeightNodes) == 0) and (size(cargoWeightNodes) == 0))) {
    logprint(4, "This aircraft does not seem to use the standard weights system. Configure payload manually.");
    return nil;
  }
  ...
  var distribute = func(what, nodes, unallocated) {
    ...
    if (((remaining > 0) and (totalF > 0))) {
      foreach (var node; nodes) {
        var maxAdd = (node.getValue("max-lb") - node.getValue("min-lb"));
        var f = (maxAdd / totalF);
        var toAdd = math.min((f * remaining), maxAdd, unallocated);
        node.setValue("weight-lb", (node.getValue("weight-lb") + toAdd));
        unallocated = (unallocated - toAdd);
      }
    }
    logprint(3, sprintf("Remaining unallocated %s: %1.1f lbs", what, unallocated));
    return unallocated;
  }
...
var setAloftWinds = func(aloftPoint) {
  forindex (var i; aloftPoint.layers) {
    ...
    logprint(2, sprintf("ALOFT AFTER %5i\': %03i/%i", node.getChild("elevation-ft").getValue(), node.getChild("wind-from-heading-deg").getValue(), node.getChild("wind-speed-kt").getValue()));
  }
}
...
var interpolateLayers = func(f, a, b) {
  ...
  return {alt : interpolate(f, a.alt, b.alt), spd : interpolate(f, a.spd, b.spd), temp : interpolate(f, a.temp, b.temp), dir : interpolateDegrees(f, a.dir, b.dir)};
}
var interpolateAloftPoints = func(f, a, b) {
  ...
  return {layers : interpolateComponentWise(f, interpolateLayers, a.layers, b.layers)};
}
var updateAloft = func() {
  ...
  var sorted = sort(aloftPoints, func(a, b) {
    return (a.dist - b.dist);
  });
  if (((sorted == nil) or (size(sorted) < 2))) {
    return nil;
  }
  var pointA = sorted[0];
  var pointB = sorted[1];
...
var importWindsAloft = func(ofp) {
  ...
  foreach (var ofpFix; ofpFixes) {
    ...
    _try(func() {
      var windData = ofpFix.getNode("wind_data");
      if ((windData == nil)) {
        return nil;
      }
      ...
      var layers = [];
      var uneven = 0;
      foreach (var ofpWindLayer; windData.getChildren("level")) {
      ...
      var name = ofpFix.getNode("ident").getValue();
      append(aloftPoints, {coord : aloftPos, dist : 0.0, layers : layers, name : name});
    }, func(errs) {});
  }
  startAloftUpdater();
}
var loadFP = func() {
  if ((getFlightplan == nil)) {
    if ((typeof(globals["getFlightplan"]) == "func")) {
      logprint(3, "Found existing getFlightplan function");
  ...
  if ((contains(globals, "commitFlightplan") and (typeof(globals["commitFlightplan"]) == "func"))) {
    ...
    globals["commitFlightplan"] = commitFlightplan;
  }
  ...
  download(username, func(filename) {
    ...
    if ((getprop("/sim/simbrief/options/import-fp") or 0)) {
      var modFP = getFlightplan(1);
      var fp = toFlightplan(ofpNode, modFP);
      if ((fp == nil)) {
...
var main = func(addon) {
  if ((globals["simbrief"] != nil)) {
    ...
    globals["simbrief"] = {loadFP : loadFP, startAloftUpdater : startAloftUpdater, stopAloftUpdater : stopAloftUpdater};
    var myMenuNode = findMenuNode(1);
    myMenuNode.setValues({enabled : "true", name : "addon-simbrief", label : "SimBrief Import", binding : {command : "dialog-show", "dialog-name" : "addon-simbrief-dialog"}});
